...
const { warehouseValidationRules } = require('../validators/warehouseValidator');
...
router.post('/', authenticateToken, warehouseValidationRules(), validate, async (req, res) => {
    const { companyId, code, name, description, status, image, address, address2, phone, phone2, warehouseType, warehouseArea, warehouseCapacity, warehouseVolume, warehouseRackCount, warehouseGateCount } = req.body;
    try {
        ...
        res.status(201).json({ message: 'Depo başarıyla oluşturuldu.', warehouse });
    } catch (err) {
        console.error(err);
        res.status(500).json({ message: 'Sunucu hatası.' });
    }
...
router.put('/:id', authenticateToken, warehouseValidationRules(), validate, async (req, res) => {
    const { companyId, code, name, description, status, image, address, address2, phone, phone2, warehouseType, warehouseArea, warehouseCapacity, warehouseVolume, warehouseRackCount, warehouseGateCount } = req.body;
    try {
